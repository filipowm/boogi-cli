...
import { installGatsby, installBoogiDependencies } from "../../utils/installer";
...
exports.builder = (yargs) => {
  yargs
    ...
    // .option("gatsby", {
    //   describe:
    //     "Use full Gatsby capabilities. Recommended only for advanced users as it brings a lot of complexity.",
    //   default: false,
    //   type: "boolean",
    // })
    .positional("path", {
...
const verifyBoogiApp = (ctx) => {
  if (isBoogiApp(ctx.path)) {
    throw new Error("BooGi was already initialized!");
  }
  return true;
};
  ...
  runTasks(
    [
      ...
      },
      {
        ...
        task: (ctx, task) => applyConfig(config, ctx),
      },
      {
...
const configure = (ctx) => {
  if (ctx.skipConfig === true) {
    ...
    config(ctx).collect((config) => {
      // ctx.version = config.version;
      return finalize(config, ctx);
    });
  }
};
